    ...
    }
    {
      ...
      systemd.services."backup-canary-read" = {
        ...
        script = ''
          ...
          d="$(mktemp -d)"
          echo "Working in $d"
          trap "rm -fr '$d' && trap - SIGTERM && kill -- -$$" SIGINT SIGTERM EXIT
          pushd "$d" || exit 1
          ...
          diff ./rascal/storage/backups/canary/current ./benland/storage/backups/canary/current
          diff ./rascal/storage/backups/canary/current ./local/storage/backups/canary/current
